...
class ParseData:
    ...
    def parse_num_string_to_byte(self, num_str):
    ...
    def parse_num_to_byte(self, num, length=1):
...
class BoxState:
    ...
    def generate_Tn_data(self, slot, rfid=None, remain_len=0.0, color=0, material_type=None):
    ...
    def get_Tn_data(self, slot):
        """Get Tn data for a slot."""
        return self.tn_data.get(slot, {})

    def modify_Tn_data(self, slot, key, value):
        """Modify specific Tn data field."""
        if slot in self.tn_data:
            self.tn_data[slot][key] = value

    def generate_Tn_inner_data(self, slot, filament_present=False, connected=False):
    ...
    def update_same_material_list(self, material_list):
...
class CutSensor:
    ...
    def __init__(self, config, box_action):
        self.printer = config.get_printer()
...
class BoxAction:
    ...
    def send_data(self, data, timeout=1.0):
    ...
    def ret_parse_process(self, response, expected_func=None):
    ...
    def communication_get_box_state(self, addr, timeout=1.0):
    ...
    def communication_get_rfid(self, addr, slot):
        """Get RFID for a slot."""
    ...
    def communication_get_remain_len(self, addr, slot):
    ...
    def communication_extrude_process(self, addr, slot):
    ...
    def communication_extrude2_process(self, addr, slot, length):
    ...
    def communication_retrude_process(self, addr, slot):
    ...
    def communication_set_pre_loading(self, addr, slot, enable):
    ...
    def communication_measuring_wheel(self, addr, slot):
    ...
    def box_extrude_material(self, tnn, use_flush=True):
    ...
    def box_extrude_material_part(self, tnn, length):
    ...
    def box_retrude_material(self, tnn):
    ...
    def box_retrude_material_filament_err_part(self, tnn):
    ...
    def z_move(self, z_pos, velocity=10.0):
        """Move Z axis."""
        if self.toolhead:
            self.toolhead.manual_move([None, None, z_pos], velocity)
            self.toolhead.wait_moves()

    def z_down(self, distance, velocity=10.0):
    ...
    def material_flush(self, from_tnn, to_tnn, length=None):
    ...
    def cal_flush_list(self, used_tnns):
    ...
    def get_flush_len(self, from_tnn, to_tnn):
    ...
    def get_flush_temp(self, from_tnn, to_tnn):
    ...
    def get_flush_max_temp(self, from_tnn, to_tnn):
    ...
    def get_flush_velocity(self, from_tnn, to_tnn):
    ...
    def check_same_box(self, tnn1, tnn2):
    ...
    def material_volume_to_length(self, volume, diameter=1.75):
    ...
    def set_cool_temp(self, temp):
        """Set cooling target temperature."""
        self.gcode.run_script_from_command("M104 S%d" % int(temp))

    def quickly_wait_heating(self, target_temp, timeout=60):
    ...
    def extruder_extrude(self, length, speed):
        """Perform extruder extrusion."""
        self.gcode.run_script_from_command(
            "G1 E%.2f F%d" % (length, int(speed * 60))
        )

    def check_and_extrude(self, length, velocity):
    ...
    def extrusion_all_materials(self, tnns, length=50.0):
    ...
    def extrude_process_stage7(self, tnn):
    ...
    def filament_conflict_check(self):
        """Check for filament conflicts."""
        return False

    def filament_err_tighten_up_event(self):
    ...
    def check_printing_used_material(self, tnns):
    ...
    def convert_scv(self, value):
        """Convert SCV value."""
        return value

    def convert_tcv(self, value):
        """Convert TCV value."""
        return value

    def generate_auto_get_rfid_func(self, slot):
        """Generate auto RFID getter function."""
        def function():
            return self.communication_get_rfid(self.addr, slot)
        return function

    def update_filament_pos(self, slot, pos):
    ...
    def update_Tnn_map(self, tnn_index, slot_index):
        """Update Tnn to slot mapping."""
        self.boxstate.generate_Tnn_map(tnn_index, slot_index)

    def update_tn_save_data(self, slot, data):
        """Update Tn save data."""
    ...
    def is_use_ending_material_flush(self):
...
class MultiColorMeterialBoxWrapper:
    ...
    def material_change_flush(self, from_tnn, to_tnn):
        """Perform material change flush."""
        self.box_action.material_flush(from_tnn, to_tnn)

    def flush_material(self, from_tnn, to_tnn):
    ...
    def get_flush_length_from_gcode(self, gcode_file):
    ...
    def flush_err_retry_process(self, from_tnn, to_tnn):
